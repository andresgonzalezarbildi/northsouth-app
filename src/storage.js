...
const DATA_KEY = 'northsouth:data:v1';
const DEVICE_KEY = 'northsouth:device-id:v1';
const LEGACY_CONFIG_KEY = 'northsouth:drive-config:v1';
...
const defaultProducts = timestamp => [
  { id:'prod-licuado', name:'Licuado', emoji:'🥤', price:0, active:true, createdAt:timestamp, updatedAt:timestamp, deletedAt:null },
  { id:'prod-alfajor', name:'Alfajor', emoji:'🍫', price:0, active:true, createdAt:timestamp, updatedAt:timestamp, deletedAt:null },
  { id:'prod-kimono', name:'Kimono', emoji:'🥋', price:0, active:true, createdAt:timestamp, updatedAt:timestamp, deletedAt:null },
  { id:'prod-agua', name:'Agua', emoji:'💧', price:0, active:true, createdAt:timestamp, updatedAt:timestamp, deletedAt:null }
];
...
export function normalizeData(input) {
  ...
  const products = Array.isArray(data.products) ? data.products : defaultProducts(timestamp);
  ...
  return {
    ...
    payments: (Array.isArray(data.payments) ? data.payments : []).filter(x => x?.id).map(p => ({
      ...p,
      amount: numberOr(p.amount),
      createdAt: p.createdAt || p.paidAt || p.updatedAt || timestamp,
      updatedAt: p.updatedAt || p.createdAt || p.paidAt || timestamp,
      deletedAt: p.deletedAt || null
    })),
    products: products.filter(x => x?.id).map(p => ({
      ...p,
      name: String(p.name || ''), emoji: String(p.emoji || '🛒'), price: Math.max(0, numberOr(p.price)),
      active: p.active !== false,
      createdAt: p.createdAt || p.updatedAt || timestamp,
      updatedAt: p.updatedAt || p.createdAt || timestamp,
      deletedAt: p.deletedAt || null
    })),
    sales: (Array.isArray(data.sales) ? data.sales : []).filter(x => x?.id).map(s => ({
      ...s,
      quantity: Math.max(1, numberOr(s.quantity, 1)), unitPrice: Math.max(0, numberOr(s.unitPrice)),
      ...
      updatedAt: s.updatedAt || s.createdAt || s.soldAt || timestamp,
      deletedAt: s.deletedAt || null
    })),
    meta: {
      ...(data.meta || {}),
      localRevision: numberOr(data.meta?.localRevision, 0),
...
export function loadData() {
  ...
  const first = normalizeData(clone(seedData));
  return saveData(first, { markDirty: false });
}

export function saveData(data, { markDirty = true } = {}) {
  ...
  // Verificación simple para detectar navegadores/modos que impidan persistir.
  if (localStorage.getItem(DATA_KEY) !== serialized) throw new Error('No se pudo confirmar el guardado local.');
  return normalized;
}

export function getLegacyDriveConfig() {
  try { return JSON.parse(localStorage.getItem(LEGACY_CONFIG_KEY) || '{}'); }
  catch { return {}; }
}

export function saveConnectedDriveAccount(email = '') {
  const current = getLegacyDriveConfig();
  localStorage.setItem(LEGACY_CONFIG_KEY, JSON.stringify({ ...current, connectedEmail: email }));
}
...
export function exportBackup(data) {
  ...
  a.href = url;
  a.download = `north-south-backup-${new Date().toISOString().slice(0,10)}.json`;
  a.click();
